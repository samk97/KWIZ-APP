      ...
      <div class="flex w-full h-screen">
        ...
        <div class="md:flex flex-col md:items-center md:w-2/12 md:bg-gray-300 md:border-r-2 md:border-gray-400">
          ...
          <div className="hidden md:flex md:w-full md:h-auto md:bg-blue-200 p-2">
            <p class="w-full">
              Start Time: <sapn>{new Date(data.startTime).toLocaleString()}</sapn>
            </p>
          </div>
          ...
          <div className="hidden md:flex md:w-full md:h-auto md:bg-blue-200 p-2">
            <p class="w-full">
              Total: <span>={data && data.questions ? data.questions.length :"0"}</span>
            </p>
          </div>
        </div>
        {/* Quiz area */}
        <div class="md:w-10/12 bg-gray-200">
        {data && data.questions &&  data.questions.map((res,num) => {
          return (
           

              <StudentQuestionArea id={res} number={num}></StudentQuestionArea>
